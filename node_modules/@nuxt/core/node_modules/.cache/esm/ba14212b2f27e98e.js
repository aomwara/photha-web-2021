let STRATEGIES,formatMessage;_2f2‍.x([["createExtendRoutesHook",()=>createExtendRoutesHook],["buildHook",()=>buildHook]]);_2f2‍.w("../helpers/constants",[["STRATEGIES",["STRATEGIES"],function(v){STRATEGIES=v}]]);_2f2‍.w("../templates/utils-common",[["formatMessage",["formatMessage"],function(v){formatMessage=v}]]);
 ...
 */
       function createExtendRoutesHook (options) {
 ...
 */
  ...
  if (options.strategy === STRATEGIES.NO_PREFIX && options.differentDomains) {
    ...
    _2f2‍.g.console.warn(formatMessage('The `differentDomains` option and `no_prefix` strategy are not compatible. Change strategy or disable `differentDomains` option.'))
  }

  if ('forwardedHost' in options) {
    // eslint-disable-next-line no-console
    _2f2‍.g.console.warn(formatMessage('The `forwardedHost` option is deprecated. You can safely remove it. See: https://github.com/nuxt-community/i18n-module/pull/630.'))
  }
  ...
  if (options.vueI18nLoader) {
    this.extendBuild(config => {
      if (!config.module) {
        _2f2‍.g.console.warn(formatMessage('Failed to register the vue-i18n-loader.'))
        return
      }
